...
def prediction_ui(gt):
    ...
    if submit and noise_level>=10:
            ...
            st.write("""\n After optimization the size of the DnCNN  models are reduced by 5 MB and have the same performance(PSNR) as the original model.
                    But here time taken by lighter version is more because we perform prediction on batch of patches thus for each patch the lite version need
